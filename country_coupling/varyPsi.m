...
beta1  = [1.25e-7; 2.68e-10; 4.26e-7];
beta2  = [4.22e-7; 6.38e-8; 7.5e-7];
beta3  = [6.59e-9; 3.57e-9; 2.66e-11];
delta  = [1/9;      1/9;       1/9     ];  %1/incubation period for Ebola Virus
gamma1 = [1/10;      1/10;       1/10     ];
gamma2 = [.0528; .0276; .0307];
psi    = [.497; 1.44; 2.21];
rho1   = [.77; .649; .451];
rho2   = [.7; .59; .41];
...
%plot(t,y(:,9),'y-',t,y(:,10)+y(:,11),'r-.',t,y(:,12)+y(:,13),'k-',t,y(:,14),'b-',guin_data(:,1), ...
%guin_data(:,2),'r*',guin_data(:,1), guin_data(:,3), 'k.','MarkerSize',10)
%plot(t,y(:,16),'y-',t,y(:,17)+y(:,18),'r-.',t,y(:,19)+y(:,20),'k-',t,y(:,21),'b-',sl_data(:,1), ...
%sl_data(:,2),'r*',sl_data(:,1), sl_data(:,3), 'k.','MarkerSize',10)
...
%xlabel('Days')
%ylabel('Population')
%print('-painters', '-depsc', '/home/eric/ebola/country_coupling/sl.eps')

%plot(t,y(:,2),'y-',t,y(:,9),'r-.',t,y(:,16))
...
%        index = find(t == temp_t);
%        err = sqrt((lib_data(i, 3) - (y(index,5) + y(index, 6)))^2 + (lib_data(i,2) - ...
...
%    if (temp_t > first_day_all)
%        index = find(t == temp_t);
%        err = sqrt((guin_data(i, 3) - (y(index,12) + y(index, 13)))^2 + (guin_data(i,2) - ...
...
%    if (temp_t > first_day_all)
%        index = find(t == temp_t);
%        err = sqrt((sl_data(i, 3) - (y(index,19) + y(index, 20)))^2 + (sl_data(i,2) - ...
